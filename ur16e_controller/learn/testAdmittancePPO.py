...
from ur16e_controller.learn.PPO import PPO
...
def main():
    ...
    trajectory = np.loadtxt("../data/20220426_092046.csv", delimiter=',', skiprows=1)
    initial_point = np.hstack([trajectory[0, 7:13], np.zeros(6)])
    env = gym.make("Admittance-v0", trajectory=trajectory, initial_point=initial_point, Render=True)
    env.print_info()
    env.seed(seed)
