...
from ur16e_controller.controllers.interpolator.base_interpolator import Interpolator
    ...
    def get_interpolated_goal(self):
        ...
        if self.ori_interpolate is not None:
            ...
            if self.ori_interpolate == "euler":
                ...
                x = trans.mat2quat(trans.euler2mat(x))
                goal = trans.mat2quat(trans.euler2mat(self.goal))

            # Interpolate to the next sequence
            x_current = trans.quat_slerp(x, goal, fraction=(self.step + 1) / self.total_steps)
            if self.ori_interpolate == "euler":
                # Map back to euler
                x_current = trans.mat2euler(trans.quat2mat(x_current))
        else:
