...
class admittance_control:
    def __init__(self, trajectory, m=100, k=1, expect_force=-10):
        """
        尝试六自由度的导纳控制
        """
        # b = 2 * 1 * math.sqrt(m  k)
    ...
    def move(self):
        ...
        for i in range(len(self.trajectory) - 1):
            ...
            delta_p = pre_p - self.trajectory[i + 1, 0:3]  # 计算与下一时刻位置偏差

            delta_vel = pre_v - desire_vel[i + 1, :]

            print("vel:{},position:{}".format(pre_v - cur_pvel, pre_p - cur_position))
            ...
            # dd_p =desire_acc[i + 1, :] + np.dot(np.linalg.inv(self.M), (delta_F_base - B - K))  # 计算加速度
            # d_p = pre_v + dd_p * 0.02  # 加速度积分计算速度
            # temp = d_p * 0.02  # 速度积分计算位置
            # delta_temp = np.dot(rotateMatrix.transpose(), temp)
            # delta_temp[0], delta_temp[1] = 0, 0
            # delta_distance = np.dot(rotateMatrix, delta_temp)
            # pose = np.hstack([position, orientation])
            # pose = pre_p[2] + delta_distance
            #

            # dd_p = np.dot(np.linalg.inv(self.M), (delta_F_base - B - K))  # 计算加速度
            # d_p = pre_v + dd_p * 0.02  # 加速度积分计算速度
            # position = pre_p + d_p * 0.02  # 速度积分计算位置
            # pose = np.hstack([position, orientation])
            pre_p = pose[0:3]
